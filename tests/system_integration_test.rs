...
fn test_new_subsystems_headless_initialization() {
    ...
    let lock_res = system_monitor::storage::file_locks::find_locking_processes(exe.to_str().unwrap(), &cancel);
    assert_eq!(lock_res.path, exe.to_str().unwrap());

    // 2. Verify cache scanner finds categories without error
    let cats = system_monitor::storage::reclaimer::scan_reclaimable_caches(&cancel);
    ...
    println!("Found {} crash reports", crashes.reports.len());
