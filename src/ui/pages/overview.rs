...
pub(crate) fn show(app: &mut crate::SystemMonitorApp, ui: &mut egui::Ui, data: &SystemData) {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        for (i, (accent, label, value, sub, frac, color)) in cards.iter().enumerate() {
            ...
            // Accent dot
            ui.painter()
                .circle_filled(cr.min + egui::vec2(16.0, 18.0), 3.0, *accent);
            ...
            ui.group(|ui| {
                ...
                ui.columns(cols, |col_uis| {
                    for (i, core) in data.cpu_cores.iter().take(cols).enumerate() {
                        ...
                        col_uis[i].label(
                            egui::RichText::new(format!("C{}", core.core_id))
                                .size(9.0)
        ...
        // ── Startup Health ──
        {
            ...
            ui.group(|ui| {
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("STARTUP HEALTH")
                            .size(10.0)
                            .color(ThemePalette::TEXT_DIMMED),
                    );
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.separator();
                    ui.label(
                        egui::RichText::new(format!("{} startup items", total)).color(ThemePalette::TEXT_LABEL_SUB),
                    );
                });
            });
        }
