...
mod tests {
    ...
    fn first_sample_is_zero() {
        assert_eq!(
            counter_rate(None, 100, Duration::from_secs(1)),
            CounterRate {
                value_per_second: 0.0,
                initialized: false
            }
    ...
    fn computes_delta_rate() {
        assert_eq!(
            counter_rate(Some(100), 300, Duration::from_secs(2)).value_per_second,
            100.0
        );
    }
    ...
    fn counter_reset_does_not_spike() {
        assert_eq!(
            counter_rate(Some(300), 100, Duration::from_secs(1)).value_per_second,
            0.0
        );
    }
